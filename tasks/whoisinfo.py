...
def whoisinfo(host):
    ...
    if not is_registered(host):
        print(f'[{Fore.RED}!{Style.RESET_ALL}] {Fore.YELLOW}{host}{Style.RESET_ALL} {Fore.RED}is not registered.{Style.RESET_ALL}')
        sys.exit()
    else:
        try:
